...
export default function GroupDetailPage() {
  ...
  const handleRemoveMember = async () => {
    ...
    try {
      const { error } = await removeMember(group.id, selectedMember.user_id, user.id);
  ...
  const handleLeaveGroup = async () => {
    ...
    try {
      ...
      const { error } = await leaveGroup(group.id, user.id);
    ...
    <div className="max-w-lg mx-auto">
      ...
      {selectedMember && !showRemoveConfirm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-4" onClick={() => setSelectedMember(null)}>
          <div className="bg-white rounded-3xl w-full max-w-sm p-6 space-y-5 animate-slide-up" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
